...
`src/lib/merkle-client.test.ts` is a **parity suite**: it pins golden vectors
generated by `python/merkle.py` (the 7-part reference tree) and asserts the
browser implementation produces the identical leaf hashes, root, and per-leaf
proof bytes — then verifies each proof. This matters because the on-chain
`MerkleAnchorRegistry.verifyInclusion` mirrors the same sorted-pair SHA-256
algorithm, so a root computed in the browser must verify on Arc unchanged. Any
drift between the three implementations (Python prover ↔ TS client ↔ Solidity
verifier) is a correctness bug the suite catches.

Regenerate the golden vectors if you ever change the algorithm — the command is
